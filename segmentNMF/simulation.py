...
def generate_artificial_NMF_data(dims: int = 2, n_cells=[6] * 2, cell_radius: int = 5, intercell_distance: int = 1,
                                 T: int = 1000, decay_step: float = .9, decay_range: int = 10,
                                 intrinsic_noise: bool = True, random_noise: float = .01):
    pad = decay_range * 2 + 10
    ...
    # Get total view size
    view_size = (cell_radius * 2 + intercell_distance / 2) * np.array(n_cells) + intercell_distance * 2 + pad
    view_size = view_size.astype('int')

    # Create cell center coordinates
    cell_centers = np.meshgrid(*[np.arange(intercell_distance + cell_radius + pad / 2,
                                           view_size[d_i] - (intercell_distance + cell_radius + pad / 2),
    ...
    for m_i, ctr in tqdm(enumerate(cell_centers.T)):
        for i in range(decay_range):
            ...
            d = decay_step ** i
            for ind in inds:
                sl = tuple([m_i] + [ind[j] - i for j in range(dims)])
                dff[sl] = F[m_i] * d

    dff = np.mean(dff, axis=0)

    if random_noise is not None:
        dff += np.random.normal(0, random_noise, size=(dff.shape))
